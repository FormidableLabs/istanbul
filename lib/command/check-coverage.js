...
Command.mix(CheckCoverageCommand, {
    ...
    usage: function () {
        ...
        console.error('Per-file and pattern-based thresholds can be specified via a configuration file.\n');
    ...
    run: function (args, callback) {
        ...
        filesFor({
            ...
            Object.keys(thresholds.global).forEach(function (key) {
                var actual = actuals[key].pct,
                    actualUncovered = actuals[key].total - actuals[key].covered,
                    threshold = thresholds.global[key];

                if (threshold < 0) {
                    if (threshold * -1 < actualUncovered) {
                        errors.push('ERROR: Uncovered count for ' + key + ' (' + actualUncovered +
                            ') exceeds global threshold (' + -1 * threshold + ')');
                    }
                } else {
                    if (actual < threshold) {
                        errors.push('ERROR: Coverage for ' + key + ' (' + actual + '%) does not meet global threshold (' + threshold + '%)');
                    }
                }
            });
